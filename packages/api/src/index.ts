...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ error: 'User already exists or signup failed', details: error.message });
  }
});
...
app.get('/projects', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.post('/projects', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const userId = req.user!.userId;
...
app.get('/projects/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.patch('/projects/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
...
app.delete('/projects/:id', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const project = await getProjectWithAccess(id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
...
app.get('/projects/:id/spec', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.patch('/projects/:id/spec', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.patch('/projects/:id/entities/:entityId', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.patch('/projects/:id/endpoints/:endpointId', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.post('/projects/:id/features', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const project = await getProjectWithAccess(id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
...
app.post('/projects/:id/preview-change', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.post('/projects/:id/apply-change', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const project = await getProjectWithAccess(id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
...
app.get('/projects/:id/export', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const project = await getProjectWithAccess(id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
...
app.post('/projects/:id/export/github', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.get('/projects/:id/export/cursor', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.get('/versions/:vId/diff', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.post('/projects/:id/invites', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.post('/projects/:id/invites/:inviteId/accept', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.post('/projects/:id/lock', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.delete('/projects/:id/lock', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.post('/projects/:id/features/:featureId/reviews', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.post('/projects/:id/features/:featureId/reviews/:reviewId/decision', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.get('/projects/:id/adrs', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.post('/projects/:id/adrs', authenticateJWT, async (req: AuthRequest, res: Response) => {
  const project = await getProjectWithAccess(req.params.id, req.user!.userId);
  if (!project) {
    return res.status(404).json({ error: 'Project not found' });
  }
...
app.patch('/projects/:id/adrs/:adrId', authenticateJWT, async (req: AuthRequest, res: Response) => {
...
app.post('/llm/suggestions', authenticateJWT, async (req: AuthRequest, res: Response) => {
